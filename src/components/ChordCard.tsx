...
export const ChordCard: React.FC<ChordCardProps> = ({ chord, sIdx, cIdx }) => {
    ...
    return (
        <>
            <div
                className={`group/card relative aspect-square flex flex-col justify-center rounded-2xl p-4 transition-all cursor-pointer border border-transparent 
                ${isPlaying
            ...
            >
                ...
                <div className={`absolute top-2 right-3 flex gap-1`}>
                    <div className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${isPlaying ? 'text-white/80 bg-white/20' : 'text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-black/20'
                        }`}>
                        {getNotation()}
                    </div>
                    {/* Duration Badge */}
                    {(chord.duration && chord.duration !== 4) && (
                        <div className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${isPlaying ? 'text-white/80 bg-white/20' : 'text-blue-500 bg-blue-100 dark:bg-blue-900/30 dark:text-blue-300'
                            }`}>
                ...
                <div className="text-center mt-3 mb-1">
                    <div className={`text-2xl font-bold tracking-tight ${isPlaying ? 'text-white' : 'text-gray-800 dark:text-white'}`}>
                        {chord.symbol}
                    </div>
                    <div className={`text-[10px] font-medium uppercase tracking-wider ${isPlaying ? 'text-white/70' : 'text-gray-400'}`}>
